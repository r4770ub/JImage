...
public class CannyEdgeFilter {
	...
	public CannyEdgeFilter(JImage sourceImage) {
		this.sourceImage = sourceImage;
		this.lowThreshold = 1.5f;
		this.highThreshold = 7.5f;
		this.gaussianKernelRadius = 2f;
		this.gaussianKernelWidth = 8;
		this.contrastNormalized = false;
	}

	public CannyEdgeFilter(JImage sourceImage, float lowThreshold, float highThreshold) {
	...
	public CannyEdgeFilter(JImage sourceImage, float lowThreshold, float highThreshold, float gaussianKernelRadius,
	...
	public void setEdgesImage(JImage edgesImage) {
	...
	private void readLuminance() {
		int type = sourceImage.getBufferedImage().getType();
		if (type == BufferedImage.TYPE_INT_RGB || type == BufferedImage.TYPE_INT_ARGB) {
			int[] pixels = (int[]) sourceImage.getBufferedImage().getData().getDataElements(0, 0, width, height, null);
			for (int i = 0; i < picsize; i++) {
				int p = pixels[i];
				int r = (p & 0xff0000) >> 16;
				int g = (p & 0xff00) >> 8;
			...
			byte[] pixels = (byte[]) sourceImage.getBufferedImage().getData().getDataElements(0, 0, width, height,
					null);
			for (int i = 0; i < picsize; i++) {
				data[i] = (pixels[i] & 0xff);
			}
		} else if (type == BufferedImage.TYPE_USHORT_GRAY) {
			short[] pixels = (short[]) sourceImage.getBufferedImage().getData().getDataElements(0, 0, width, height,
					null);
			for (int i = 0; i < picsize; i++) {
			...
			BufferedImage temp = new BufferedImage(sourceImage.getWidth(), sourceImage.getHeight(),
					BufferedImage.TYPE_INT_RGB);
			Graphics graph = temp.createGraphics();
			graph.drawImage(sourceImage.getBufferedImage(), 0, 0, null);
			graph.dispose();
			sourceImage = new JImage(temp);
			int[] pixels = (int[]) sourceImage.getBufferedImage().getData().getDataElements(0, 0, width, height, null);
	...
	private void writeEdges(int pixels[]) {
		...
		temp.getWritableTile(0, 0).setDataElements(0, 0, width, height, pixels);
		edgesImage = new JImage(temp);
	}
